<?php function upload(){global $upload_services;$d=opendir(HOST_DIR.'upload/');while(($modules=readdir($d))!==false){if(substr($modules,0,1)!='.'&&preg_match('/\.index\.php$/i',$modules)&&is_file(HOST_DIR."upload/$modules"))include_once(HOST_DIR."upload/$modules");}if(empty($upload_services)){echo '<span class="warning"><b>'.lang(48)."</b></span>";return;}sort($upload_services);reset($upload_services);$uploadtype='';foreach($upload_services as $upl)$uploadtype.="['$upl',".(!empty($max_file_size[$upl])?$max_file_size[$upl]:0)."],";$uploadtype=substr($uploadtype,0,-1);echo "<script type='text/javascript'>/* <![CDATA[ */var upservice=[$uploadtype];function fill_option(id){var elem=document.getElementById(id);for(var i=0;i<upservice.length;i++){elem.options[elem.options.length]=new Option(upservice[i][0].replace('_',' ')+(upservice[i][1]?' ('+upservice[i][1]+' MB)':''));elem.options[elem.options.length-1].value=upservice[i][0]}}function openwinup(id){var options='width=700,height=250,toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,copyhistory=no';win=window.open('',id,options);win.focus();return true}/* ]]> */</script>\n<table align='center' border='0'>\n";for($i=0;$i<count($_GET['files']);$i++){$file=$GLOBALS['list'][($_GET['files'][$i])];$tid=md5(time().'_file'.$_GET['files'][$i]);?>
<tr>
